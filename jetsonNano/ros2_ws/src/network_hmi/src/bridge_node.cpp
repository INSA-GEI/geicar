...
public:
  ...
  : Node("tcp_udp_bridge")
  {
    ...
    publisher_joystick_order_ = this->create_publisher<interfaces::msg::JoystickOrder>(
      "joystick_order", 1);
...
private:
  ...
  // Helper to send a JSON/text message over a TCP socket with a delimiter
  // Ensures whole buffer is sent and appends a '\n' so clients can split messages
  ...
  void client_session(int client_socket, std::string client_ip)
  {
    ...
    RCLCPP_INFO(
      this->get_logger(), "Nouvelle session client depuis %s (socket %d)",
    ...
    std::shared_ptr<std::atomic<long long>> last_activity_ms =
    ...
    while (rclcpp::ok()) {
      ...
      if (valread <= 0) {
        ...
        if (valread < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
          ...
          if (last_ms != 0 && (now - last_ms) > TIMEOUT_MS) {
            RCLCPP_WARN(
              this->get_logger(), "Timeout d'activité (%lld ms) — fermeture de la session",
              now - last_ms);
            break;
          } else {
      ...
      try {
        ...
        if (msg.contains("type") && msg["type"] == "register") {
          ...
          RCLCPP_INFO(
            this->get_logger(), "Requête d'enregistrement '%s' depuis %s (port retour UDP: %d)",
          ...
          // Enregistrement pour le modèle à client unique
          {
            ...
            if (single_client_connected_) {
              ...
              send_tcp_message(client_socket, resp_str);
              continue;
            }
          ...
          std::thread(
            [this, weak_alive, weak_last, hb_socket, TIMEOUT_MS]() {
              ...
              while (alive && alive->load() && rclcpp::ok()) {
                ...
                if (last_ms != 0 && (now - last_ms) > TIMEOUT_MS) {
                  RCLCPP_WARN(
                    rclcpp::get_logger("tcp_udp_bridge"),
                    "Client non-réactif depuis %lld ms, fermeture de la session", now - last_ms);
          ...
          RCLCPP_INFO(
            this->get_logger(), "Changement de mode reçu de %s : %d",
          ...
          RCLCPP_WARN(
            this->get_logger(), "Message TCP inconnu de %s: %s",
        ...
        RCLCPP_WARN(
          this->get_logger(), "Échec du parsing JSON TCP (session %s) : %s",
    ...
    RCLCPP_INFO(
      this->get_logger(), "Session client %s (socket %d) terminée",
      client_ip.c_str(), client_socket);
  }
  ...
  void udp_data_loop()
  {
    ...
    while (rclcpp::ok()) {
      ...
      int n = recvfrom(
        udp_data_socket_, (char *)buffer, 1024, MSG_WAITALL,
  ...
  void register_client(const std::string & client_id, const std::string & ip, int recv_port)
  {
    ...
    if (single_client_connected_) {
      RCLCPP_WARN(
        this->get_logger(), "Tentative d'enregistrer un second client '%s' — refusé",
        client_id.c_str());
      return;
    }
    single_client_connected_ = true;
    ...
    single_client_recv_port_ = recv_port;
    RCLCPP_INFO(
      this->get_logger(), "Client unique enregistré: %s (%s:%d)",
      client_id.c_str(), ip.c_str(), recv_port);
  }
