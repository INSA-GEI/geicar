...
// Mutex pour protéger l'accès aux timers
static SemaphoreHandle_t s_timers_mutex;
...
static void TimerService_Task(void *argument) {
    ...
    for (;;) {
        ...
        if (xSemaphoreTake(s_timers_mutex, portMAX_DELAY) == pdTRUE) {
            ...
            // Relâche le mutex
            xSemaphoreGive(s_timers_mutex);
        } else {
            // Erreur : impossible de prendre le mutex. Cela ne devrait pas arriver avec portMAX_DELAY.
            // Tu peux ajouter une logique de gestion d'erreur ici si nécessaire.
            // Par exemple, envoyer un message d'erreur via ITM ou un UART de debug.
        }
    }
}
...
BaseType_t SW_TIMER_Init(void) {
    ...
    // 2. Crée le mutex pour protéger les accès aux timers
    s_timers_mutex = xSemaphoreCreateMutex();
    if (s_timers_mutex == NULL) {
        // Gérer l'erreur : pas assez de mémoire pour le mutex
        return pdFALSE;
    }
    vQueueAddToRegistry(s_timers_mutex, "SW_TIMERS_MUTEX");
    ...
    if (xTaskCreate(TimerService_Task,
        ...
        vSemaphoreDelete(s_timers_mutex); // Nettoyage
        return pdFALSE;
    }
